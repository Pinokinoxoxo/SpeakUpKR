...
namespace SpeakUp
{
	public static class ExtraGrammarUtility
	{
        ...
        public static IEnumerable<Rule> ExtraRules()
        {
            ...
            if (initiator == null || !initiator.IsValid()) return Enumerable.Empty<Rule>();

            Pawn recipient = DialogManager.Recipient;
            try
            {
                ExtraRulesForPawn(initPrefix, initiator, recipient);
                if (recipient?.IsValid() == true) ExtraRulesForPawn(reciPrefix, recipient, initiator);
        ...
        public static void ExtraRulesForPawn(string symbol, Pawn pawn, Pawn other = null)
        {
            ...
            // age?
            string ageValue;
            if (SpeakUpSettings.useAgeBiologicalYears)
            {
                ageValue = pawn.ageTracker.AgeBiologicalYears.ToString();
            }
            else
            {
                ageValue = pawn.ageTracker.AgeChronologicalYears.ToString();
            }
            MakeRule(symbol + "age", ageValue);

            // Check if the other pawn (RECIPIENT) is valid and compare ages
            if (other != null && other.IsValid())
            {
                int initiatorAge = SpeakUpSettings.useAgeBiologicalYears ? pawn.ageTracker.AgeBiologicalYears : pawn.ageTracker.AgeChronologicalYears;
                int recipientAge = SpeakUpSettings.useAgeBiologicalYears ? other.ageTracker.AgeBiologicalYears : other.ageTracker.AgeChronologicalYears;

                if (initiatorAge > recipientAge)
                {
                    MakeRule(symbol + "AgeBool", "1");
                }
                else if (initiatorAge < recipientAge)
                {
                    MakeRule(symbol + "AgeBool", "0");
                }
            }
        ...
        {
            if (Prefs.DevMode) // This ensures logging only happens in DevMode.
            {
                Log.Message($"[SpeakUp] {message}");
            }
        }
